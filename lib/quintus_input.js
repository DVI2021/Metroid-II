  ...
  Q.component("platformerControls", {
    ...
    step: function(dt) {
      ...
      if(p.ignoreControls === undefined || !p.ignoreControls) {
        ...
        //He tenido que tocar esto para cambiar el salto al space
        if(p.landed > 0 && (Q.inputs['fire'] || Q.inputs['action']) && !p.jumping) {
          p.vy = p.jumpSpeed;
          p.landed = -dt;
          p.jumping = true;
        } else if(Q.inputs['space'] || Q.inputs['action']) {
          this.entity.trigger('jump', this.entity);
          p.jumping = true;
        }

        if(p.jumping && !(Q.inputs['fire'] || Q.inputs['action'])) {
          p.jumping = false;
